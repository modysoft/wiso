...
function projectAdd(txtProj,shortname,loader,btn){
	...
	if(proj1 == ""){
		...
		if(!proj1.match(/^[a-zA-Z0-9\u0600-\u06FF,-][\sa-zA-Z0-9\u0600-\u06FF,-]*$/g)){
			msg = "اسم المشروع يجب أن يبدأ بالأحرف الأبجدية أو الأرقام تجنب الرموز!";
			$('#err_msg').show();
			$('#err_msg').html(msg);
			$('#'+txtProj).focus();
			return false;
	...
	if(shortname1.trim() == ""){
		msg = "اسم المشروع القصير فقط باللغة الإنكليزية ولا يمكن أن يكون فارغاً";
		document.getElementById('err_msg').style.display = 'block';
		document.getElementById('err_msg').innerHTML = msg;
		...
		if(!isNaN(x)){
			msg = "اسم المشروع القصير لا يقبل الأرقام في نهايته";
			document.getElementById('err_msg').style.display = 'block';
			document.getElementById('err_msg').innerHTML = msg;
			document.getElementById(shortname).focus();
...
function validatemilestone(){
	...
	if(project_id.value.trim() == ""){
		errMsg = "لا يمكن ترك اسم المشروع فارغا!";
		project_id.focus();
		done = 0;

	}
	else if(title.value.trim() == "") {
		errMsg = "لا يمكن ترك حقل العنوان فارغا!";
		title.focus();
		done = 0;
	}
	else if(start_date.value.trim() == ""){
		errMsg = "لا يمكن ترك حقل تاريخ البدء فارغا!";
		start_date.focus();
		done = 0;

		}
	else if(end_date.value.trim() == ""){
		errMsg = "لا يمكن ترك حقل تاريخ الانتهاء فارغا!";
		end_date.focus();
		done = 0;

		}
	else if(Date.parse(start_date.value) > Date.parse(end_date.value)) {
		errMsg = "لا يمكن أن يتجاوز تاريخ البدء تاريخ الانتهاء!";
		end_date.focus();
		done = 0;
	}
	if(done == 0) {
...
function submitProfile() { 
	...
	if(name1== "") {
		...
		errMsg = "لا يمكن ترك اسم العائلة فارغا!";
...
function hide_popoup(resetFlds) {
	...
	if(resetFlds ==1 ) {
		...
		$("#CS_title").val('');
		$("#CS_title").val('أضف المهمة هنا ثم إضغط على الرجوع');
		$("#opt3").children("a").children("span").html('No Due Date');
		$("#opt5").children("a").children("span").html('me');
		var http_images = $("#hid_http_images").val();
		$("#opt1").children("a").children("span").html('<img class="flag" src="'+http_images+'images/types/dev.png" alt="type" style="padding-top:3px;"/>&nbsp;أعمال متابعة');
...
function checkboxrange(){
	...
	function add_cases_project(){
		...
		if(done == 0) {
			...
			var conf = confirm("هل تريد بالتأكيد إضافة مهام إلى المشروع؟");
			if(conf == true)
			{
			document.getElementById('subprof1').style.display='none';
			document.getElementById('subprof2').style.display='block';
			...
			$.post(strURL+"ajax_add_template_cases",{"pj_id":pj_id,"temp_mod_id":temp_mod_id}, function(data){
				  if(data) {
					if(data == 1){
						...
						document.getElementById('subprof2').style.display='none';
						var op = 100;
						showTopErrSucc('success','تمت إضافة المهام.');
						return false;	
					}else{
						document.getElementById('subprof1').style.display='block';
						document.getElementById('subprof2').style.display='none';
						var op = 100;
						showTopErrSucc('error','تمت إضافة النموذج بالفعل إلى هذا المشروع!');
						return false;
					}
				  }
			});
...
function getImNotifyMsg(projShName, caseNum, caseTtl, caseTyp){
	var action = '';
	switch(caseTyp) {
		case 'NEW': action = "تم إنشاء مهمة جديدة";
					break;
		case 'UPD': action = "تم تحديث المهمة";
					break;
		case 'ARC': action = "تم أرشفة المهمة";
					break;
		case 'DEL': action = "تم حذف المهمة";
					break;
		default:  	action = "تنبيه جديد";
	}
