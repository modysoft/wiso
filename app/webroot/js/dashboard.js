	...
	$.post(strURL+"case_project",{projFil:projFil,caseStatus:caseStatus,customfilter:customfilter,caseChangeAssignto:caseChangeAssignto,caseChangeDuedate:caseChangeDuedate,caseChangePriority:caseChangePriority,caseChangeType:caseChangeType,mstype:mstype,priFil:priFil,caseTypes:caseTypes,caseMember:caseMember,caseAssignTo:caseAssignTo,caseDate:caseDate,caseSearch:caseSearch,casePage:casePage,caseId:caseId,caseTitle:caseTitle,caseDueDate:caseDueDate,caseNum:caseNum,caseLegendsort:caseLegendsort,caseAtsort:caseAtsort,startCaseId:startCaseId,caseResolve:caseResolve,caseMenuFilters:caseMenuFilters,caseUrl:caseUrl,detailscount:detailscount,milestoneIds:milestoneIds,case_srch:case_srch,case_date:case_date,'case_due_date':case_due_date,caseCreateDate:caseCreateDate,projIsChange:projIsChange},function(res) {
		if(res){
			...
			if(caseId) {
				...
				if(chk != -1) {
					showTopErrSucc('success','تم إغلاق المهام.');
				}
				else {
					showTopErrSucc('success','تم إغلاق المهمة');
				}
			...
			if(startCaseId) {
				...
				if(chk != -1) {
					showTopErrSucc('success','تم البدء بالمهام.');
				}
				else {
					showTopErrSucc('success','تم البدء بالمهمة');
				}
				//$("#casePage").val('1');
				casePage = 1;
			}
			if(caseResolve) {
				...
				if(chk != -1) {
					showTopErrSucc('success','تم الإنتهاء من المهام');
				}
				else {
					showTopErrSucc('success','تم إنهاء المهمة');
				}
				//$("#casePage").val('1');
				casePage = 1;
			}
...
function validateComments(id,uniqid,legend,ses_type,pid){
	...
	if (err) {
		if ($('input[id^='+id+'jqueryfile]').length && confirm('هل تريد بالتأكيد النشر بدون رد؟')) {
			err = '';
		} else {
...
easycase.getStatus = function(type,legend){
	if(type == 10) {
		return '<div class="label update">تحديــث</div>';
	} else if(legend == 1) {
		return '<div class="label new">جديـدة</div>';
	} else if(legend == 2 || legend == 4) {
		return '<div class="label wip">متقدمــة</div>';
	}
	if(legend == 3) {
		return '<div class="label closed">مغلقــة</div>';
	} else if(legend == 4) {
		return '<div class="label wip">متقدمــة</div>';
	} else if(legend == 5) {
		return '<div class="label resolved">منتهيــة</div>';
	}
}
...
easycase.getColorStatus = function(type,legend){
	if(type == 10) {
		return '<b class="update">تحديــث</b>';
	} else if(legend == 1) {
		return '<b class="new">جديـدة</b>';
	} else if(legend == 2 || legend == 4) {
		return '<b class="wip">متقدمــة</b>';
	}
	if(legend == 3) {
		return '<b class="closed">مغلقــة</b>';
	} else if(legend == 4) {
		return '<b class="wip">متقدمــة</b>';
	} else if(legend == 5) {
		return '<b class="resolved">منتهيــة</b>';
	}
}
...
easycase.getColorPriority = function(casePriority){
	if(casePriority == "NULL" || casePriority == "") { return; }
	else if(casePriority == 0) { return '<b style="color:#FF0000;">عالي الأهمية</b>'; }
	else if(casePriority == 1) { return '<b style="color:#28AF51;">متوسط الأهمية</b>'; }
	else if(casePriority == 2){ return '<b style="color:#B4A532;">أقل أهمية</b>'; }
}
...
function startCase(id, no, dtlsid) { 
	var conf = confirm("هل أنت متأكد من أنك تريد بدء المهمة #"+no+" ؟");
	if(conf == false) {
		$('#caseStart').val("");
		//enableButtons(); not impl
		return false;
...
function caseResolve(id, no, dtlsid) {
	var conf = confirm("هل تريد بالتأكيد إنهاء المهمة #"+no+" ؟");
	if(conf == false) {
		$('#caseResolve').val("");
		//enableButtons(); not impl
		return false;
...
function setCloseCase(id, no, dtlsid) {
	var conf = confirm("هل تريد بالتأكيد إغلاق المهمة #"+no+" ؟");
	if(conf == false) {
		$('#caseId').val("");
		//enableButtons(); not impl
		return false;
...
function moveTaskToProject() {
	...
	if($('#ismultiple_move').val()==1){
		if(countJS(case_id)==1){
			var cmsg = "هل تريد بالتأكيد نقل المهمة #"+case_no[0]+" to '"+new_prj_nm+"' ؟";
		}else{
			var cmsg = "هل تريد بالتأكيد نقل هذة المهمة المحددة إلى '"+new_prj_nm+"' ؟";
		}
	}else{
		var cmsg = "هل تريد بالتأكيد نقل المهمة #"+case_no+" from '"+old_prj_nm+"' to '"+new_prj_nm+"' ؟";
	}	
...
function archiveCase(id,cno,pid,dtlsid) {
	var conf = confirm("هل تريد بالتأكيد أرشفة المهمة #"+cno+" ؟");
	if(conf == false) {
		return false;
	} else {
		refreshActvt = 1;
...
function deleteCase(id,cno,pid,dtlsid){
	if(confirm("هل تريد بالتأكيد حذف المهمة #"+cno+" ?")){
		refreshActvt = 1;
		var curRow = "curRow"+id;
		$("#"+curRow).fadeOut(500);
		$('#caseLoader').show();
...
function removeThisCase(count,msid,getcount,milestone_id,cno,uid) {//alert('jyoti');exit;
	...
	var conf = confirm("هل تريد بالتأكيد إزالة  'task #"+cno+"' من هذا المعلم؟");
...
function archiveFile(obj) {
    ...
    var conf = confirm("هل تريد بالتأكيد أرشفة الملف '" + name + "' ?");
    if (conf == false) {
	return false;
    } else {
	var curRow = "curRow" + id;
...
function removeTask(taskid,taskno,mlstid,project_id){
	var conf=confirm("هل أنت متأكد من أنك تريد إزالة #"+taskno+" مهمة من معلمها ");
        if(conf){
...
function switchTaskToMilestone(obj){
	if($('#mvtask_cnt').val()>0){
		...
		if(ext_mlst_id==curr_mlst_id){
			showTopErrSucc('error','Opps! #'+task_no+" هو موجود بالفعل في المعلم المحدد");
		}else{
			if(confirm('هل تريد بالتأكيد نقل  #'+task_no+" مهمة إلى هذا المعلم؟")){
				$.post(HTTP_ROOT+'milestones/switchTaskToMilestone',{'taskid':task_id,'curr_mlst_id':curr_mlst_id,'project_id':project_id,'ext_mlst_id':ext_mlst_id},function(res){
					if(res=='success'){
						showTopErrSucc('success','Task #'+task_no+" نقل بنجاح.");
					}else{
						showTopErrSucc('error','خطأ في نقل المهمة إلى المعلم');
					}
				});
				closePopup();
				if($('#caseMenuFilters').val()=='milestonelist'){
		...
		showTopErrSucc('خطأ','Opps! لا معلم في المشروع لنقل المهمة ');
	}
}
...
function save_editedvalue_reply(caseno,id,proj_id,repUniqId){
	...
	$.post(HTTP_ROOT+"easycases/save_editedvalue",{'id':id,'message':message,'caseno':caseno,proj_id:proj_id},function(res){
		if(res==0){
			...
			showTopErrSucc('success','تم تعديل ردك بنجاح.');
