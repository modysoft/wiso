...
function routeOSHash(){
    ...
    if(CONTROLLER == 'projects' && PAGE_NAME == 'manage') {
        $("#case_search").attr("placeholder", "البحث في المشاريع");
        $('.customFilter').html('');
        addUserToProject();
    } else if(CONTROLLER == 'users' && PAGE_NAME == 'manage') {
        $("#case_search").attr("placeholder", "البحث في المستخدمين");
        $("img.lazy").lazyload({
        ...
        switch(params[0]){
            case 'tasks':
                ...
                if(CONTROLLER == 'easycases' && PAGE_NAME == 'dashboard') {
                    if($("#caseLoader").is(':visible') == false) {
                        $("#case_search").attr("placeholder", "البحث في المهام");
            ...
            case 'files':
                ...
                if(CONTROLLER == 'easycases' && PAGE_NAME == 'dashboard') {
                    $("#case_search").attr("placeholder", "البحث في الملفات");
                    $('.customFilter').html('');
                    if($("#caseLoader").is(':visible') == false) {
                        $("#widgethideshow").hide();
            ...
            case 'milestonelist':
                if(CONTROLLER == 'easycases' && PAGE_NAME == 'dashboard') {
                    ...
                    $('#widgethideshow').hide();
                    if($("#caseLoader").is(':visible') == false) {
                        $("#case_search").attr("placeholder", "البحث في المعالم");
...
function validatechart(type){
    ...
    if(start_date.value.trim() == ""){
        errMsg = "من تاريخ  - لا يمكن يترك فارغا!";
        start_date.focus();
        done = 0;
    }
    else if(end_date.value.trim() == ""){
        errMsg = "إلى تاريخ - لا يمكن يترك فارغا!";
        end_date.focus();
        done = 0;
    }
    else if(Date.parse(start_date.value) > Date.parse(end_date.value)) {
        errMsg = "من تاريخ(From Date) لا يمكن أن يتجاوز الى التاريخ(To Date)!";
        end_date.focus();
        done = 0;
    }
    if(done == 0) {
...
function submitfilter() {
    ...
    if (filtername.trim() != "") {
        ...
        $.post(strURL + "ajax_customfilter_save", {
            ...
            if (data) {
                if (data == 'success') {
                    showTopErrSucc('success', 'تم حفظ الفلتر المخصص بنجاح ..');
                    closePopup();
                    resetAllFilters();
                    ...
                    showTopErrSucc('error', 'اسم الفلتر المخصص موجود بالفعل ..');
                    showSaveFilter();
                    $('#savefilter_name').focus();
                }
            }
...
function restoreTaskDetail(caseid, caseNo) {
    if(confirm("هل تريد بالتأكيد استعادة المهمة #"+caseNo+" ؟")) {
        var caseids = Array();
        caseids.push(caseid);
        $.post(HTTP_ROOT+"archives/move_list",{
            ...
            if(data){
                easycase.ajaxCaseDetails(caseid, 'case', 0);
                showTopErrSucc('success', "المهمة #"+caseNo+" تم إسترجاعها");
            }
        });
    }
}
...
function case_quick(easycase){ // add assignto, user checkboxs,
	...
	$('#CS_type_id').val(2);
	$('#ctsk_type').html('<img class="flag" src="'+HTTP_IMAGES+'images/types/dev.png" alt="type" style="padding-top:3px;"/>&nbsp;أعمال متابعة');
    }
...
function addUserToProject() {
    ...
    if(parseInt(prj_id)) {
        ...
        if(confirm("هل تريد إضافة مستخدم في '"+prj_name+"' ?")) {
...
function projectAdd(txtProj, shortname, loader, btn) {
    ...
    proj1 = proj1.trim();
    if (proj1 == "") {
        msg = "لا يمكن ترك اسم المشروع فارغا!";
        document.getElementById('err_msg').style.display = 'block';
        document.getElementById('err_msg').innerHTML = msg;
        document.getElementById(txtProj).focus();
        return false;
    } else {
        if (!proj1.match(/^[a-zA-Z0-9\u0600-\u06FF,-][\sa-zA-Z0-9\u0600-\u06FF,-]*$/g)) {
            msg = "اسم المشروع يجب أن يبدأ بأحرف أبجدية أو أرقام!";
            $('#err_msg').show();
            $('#err_msg').html(msg);
            $('#' + txtProj).focus();
            return false;
    ...
    if (shortname1.trim() == "") {
        msg = "لا يمكن ترك اسم المشروع القصير فارغا!";
        document.getElementById('err_msg').style.display = 'block';
        document.getElementById('err_msg').innerHTML = msg;
        ...
        if (!isNaN(x)) {
            msg = "الإسم قصير لا يمكن أن ينتهي بالأعدد أو مساحات فارغة!";
            document.getElementById('err_msg').style.display = 'block';
            document.getElementById('err_msg').innerHTML = msg;
            document.getElementById(shortname).focus();
            ...
            if (!email_id.match(emailRegEx)) {
                if (email_id.indexOf(',') != -1) {
                    for (var i = 0; i < email_arr.length; i++) {
                        if (email_arr[i].trim() != "") {
                            if ((!email_arr[i].trim().match(emailRegEx))) {
                                ...
                                msg = "بريد إلكتروني خاطئ: '" + email_arr[i] + "'";
                                $('#err_mem_email').show();
                    ...
                    if (totlalemails == email_arr.length) {
                        msg = "ما تم إدخاله ليس بريدا إلكترونيا صالحا";
                        $('#err_mem_email').show();
                        $('#err_mem_email').html(msg);
                        $('#members_list').focus();
                        return false;
        ...
        $.post(strURL + "projects/ajax_check_project_exists", {
            ...
            if (data == "Project") {
                ...
                document.getElementById(loader).style.display = 'none';
                document.getElementById(btn).style.display = 'block';
                msg = "اسم المشروع القصير موجود بالفعل!";
                document.getElementById('err_msg').style.display = 'block';
                document.getElementById('err_msg').innerHTML = msg;
                document.getElementById(shortname).focus();
                return false;
                ...
                if (email_id) {
                    $.post(strURL + 'users/check_fordisabled_user', {
                        ...
                        if (res != '1') {
                            ...
                            $('#' + btn).show();
                            if (res.indexOf(',') != -1) {
                                var msg = "'" + res + "' المستخدمون هم مستخدمون غير مفعليين. لا يسمح لهم بإضافة إلى مشروع.";
                            } else {
                                msg = "'" + res + "' هو مستخدم غير مفعل، لذلك لا يمكن أن يضاف إلى المشروع";
                            }
    ...
    if (email_id == "") {
        ...
        if (!email_id.match(emailRegEx)) {
            if (email_id.indexOf(',') != -1) {
                ...
                if (totlalemails == email_arr.length) {
                    msg = "ما تم إدخاله ليست بريدا إلكترونيا صالحا";
                    $('#err_email_new').html("");
                    $('#err_email_new').show();
...
function updateAllProj(radio, projId, page, all, pname, srch) {
    ...
    if ($('#reset_btn').is(":visible") && !$('#customFIlterId').val() && !($('#filtered_items .filter_opn').length == 1 && casePage > 1)) {
        if (confirm('هل تريد إعادة تعيين الفلاتر النشطة بالفعل؟')) {
...
function submitProfile() {
    ...
    var done = 1;
    if (name1 == "") {
        errMsg = "لا يمكن ترك حقل الاسم الأول فارغا!";
        $('#profile_name').focus();
        done = 0;
    }/*else if (last_name == "") {
        ...
        errMsg = "لا يمكن ترك حقل الاسم المختصر فارغا!";
        $('#short_name').focus();
        done = 0;
    }
...
function checkPasswordMatch(a, b, c,d) {
    ...
    if (pass.trim() != "") {
        ...
        if (pass_new.trim() == "") {
            errMsg = "لا يمكن أن تكون كلمة السر فارغة!";
		...
		/*else if (pass_new.length < 6) {
            ...
            errMsg = "تأكيد كلمة السر لا يمكن أن تكون فارغة!";
            document.getElementById(b).focus();
            done = 0;
        } else if (retypr_pass.trim() != pass_new.trim()) {
            errMsg = "كلمة السر غير متطابقة!";
            document.getElementById(b).focus();
            done = 0;
        } else {
        ...
        errMsg = "لا يمكن ترك كلمة السر القديمة فارغة!";
    ...
    $('#file_upload1').fileUploadUI({
        ...
        buildDownloadRow: function(file) {
			...
			onInit: setInfo,
                    ...
                    alert("الصورة التي حاولت تحميلها صغيرة جدا. يجب أن يكون عرضهه 100 بكسل على الأقل.\nالرجاء إعادة المحاولة باستخدام صورة أكبر.");
                    $("#profLoader").hide();
                    $("#inactConfirmbtn").show();
                    $("#actConfirmbtn").hide();
                } else if (file.message == "exceed") {
                    alert("حدث خطأ أثناء تحميل الملف.\nتجاوز مساحة التخزين من قبل " + file.storageExceeds + " Mb!");
                }
                else if (file.message == "size") {
                    alert("حدث خطأ أثناء تحميل الملف. لا يمكن أن يكون حجم الملف أكثر من  " + fmaxilesize + " Mb!");
                }
                else if (file.message == "error") {
                    alert("حدث خطأ أثناء تحميل الملف. الرجاء المحاولة بملف آخر");
                }
                else if (file.message == "s3_error") {
                    alert("بسبب بعض المشاكل في الشبكة لم يتم تحميل الملف الخاص بك. يرجى المحاولة مرة أخرى بعد وقت ما.");
                }
                else {
                    alert("نأسف، " + file.message + " نوع الملف غير مسموح به!");
                }
            } else {
                alert("حدث خطأ أثناء تحميل الملف. الرجاء المحاولة بملف آخر");
            }
...
function submitProject(proj, shrt) {
    ...
    }else if (!proj_name.match(/^[a-zA-Z0-9\u0600-\u06FF,-][\sa-zA-Z0-9\u0600-\u06FF,-]*$/g)) {
        msg = "اسم المشروع يجب أن يبدأ بالأحرف الأبجدية أو الأرقام!";
        document.getElementById(proj).focus();
        done = 0;
    }
    ...
    if (short_name == "") {
        msg = "لا يمكن ترك الإسم القصير فارغا!";
        document.getElementById(shrt).focus();
        done = 0;
    }else if(patern.test(short_name) !== true) {
        msg = "يجب أن يكون الإسم القصير فقط حروف أبجدية أو أرقام!";
        document.getElementById(shrt).focus();
        done = 0;
    }else{
        var x = short_name.substr(-1);
        if (!isNaN(x)) {
            msg = "الإسم القصير لا يمكن أن يكون ينتهي بالأرقام!";
            document.getElementById(shrt).focus();
            done = 0;
        }
    }
...
$(".del_prj").click(function(){
    ...
    if (confirm("هل أنت متأكد من حذف المشروع '" + prj_nm + "'")) {
        if (confirm("جميع المهام، الملفات المرتبطة '" + prj_nm + "' سيتم حذفها نهائيا.")) {
...
$(".enbl_prj").click(function(){
    var prj_id = $(this).attr('data-prj-id');
    var prj_name = $(this).attr('data-prj-name');
    var conf = confirm("هل تريد بالتأكيد تفعيل '"+prj_name+"' ?");
    if(conf == true) {
        var pg = $(".button_page").html();
...
$(".disbl_prj").click(function(){
    var prj_id = $(this).attr('data-prj-id');
    var prj_name = $(this).attr('data-prj-name');
    var conf = confirm("هل تريد بالتأكيد تعطيل '"+prj_name+"' ?");
    if(conf == true) {
        var pg = $(".button_page").html();
...
function removeusers() {
    ...
    if(done) {
        if (confirm("هل أنت متأكد من أنك تريد إزالة '"+user_name+"' from '"+remove_prj_name+"'?")) {
            ...
            showTopErrSucc('success', "User(s) '"+user_name+"' تمت إزالته من '"+remove_prj_name+"'");
        } else {
            return false;
        }
    }
    ...
    if (done) {
        ...
        $.post(strURL, {
            ...
            if (data == "success") {
                ...
                $("#userList").html('');
                hduserid = new Array();
                showTopErrSucc('success', tot + ' تمت إضافة المستخدم (المستخدمين) بنجاح');
...
function validateemailrpt(){
    ...
    if($('#dlyupdateyes').is(":checked")){
        ...
        if(hr == ""){
            errMsg = "لا يمكن ترك حقل الساعات فارغا";
            document.getElementById("not_hr").focus();
            done = 0;
        }else if(mn == ""){
            errMsg = "لا يمكن ترك حقل الدقائق فارغا";
            document.getElementById("not_mn").focus();
            done = 0;
        }else if(prjct == null){
            errMsg = "لا يمكن ترك حقل المشروع فارغا";
...
function submitCompany() {
    ...
    if(cmpname.trim() == "") {
        ...
        errMsg = "لا يمكن ترك حقل الاسم فارغا!";
        $("#name").focus();
        done = 0;
...
function addProjectToUser() {
    ...
    if(usr_id) {
        ...
        if(confirm("هل تريد تعيين المشروع ل '"+usr_name+"' ?")) {
...
function grantOrRemoveModerator(obj) {
    ...
    if (confirm("هل أنت متأكد أنك تريد "+msg+" المشرف ل '"+usr_name+"'?")) {
        $.post(HTTP_ROOT + "users/grant_moderate", {
            ...
            if (parseInt(data)) {
                ...
                $(obj).text(text);
                showTopErrSucc('success', suc_msg+' امتياز المشرف بنجاح');
            } else {
                showTopErrSucc('error', "خطأ في "+err_msg+" إلى مشرف");
            }
        });
    }
}
...
function removeprojects() {
    ...
    if(done) {
        if (confirm("هل أنت متأكد من أنك تريد إزالة '"+project_name+"' من '"+rmv_user_name+"'?")) {
            $('#inner_usr_prj_rmv input:checked').each(function() {
                if($(this).attr('id') !== 'checkAllprojects'){
                    var listid = $(this).attr('id');
            ...
            showTopErrSucc('success', "المشروع (المشاريع) '"+project_name+"' تمت إزالتها من '"+rmv_user_name+"'");
        } else {
            return false;
        }
    }
        ...
        $.post(strURL, {
            ...
            if (data) {
                ...
                if (parr[0] == "success") {
                    ...
                    $("#prjList").html('');
                    hdprojectid = new Array();
                    showTopErrSucc('success', 'تم تعيين المشروع بنجاح');
                }
            }
        });
    }
...
function resend_invitation(qrst, email) {
    if (confirm('هل تريد بالتأكيد إعادة إرسال دعوة بالبريد الإلكتروني إلى \'' + email + '\' ?')) {
        $("#projectLoader").show();
        $.post(HTTP_ROOT + 'users/resend_invitation', {
            ...
            if (res.msg == 'succ') {
                $("#projectLoader").hide();
                showTopErrSucc('success', "تم إرسال رابط الدعوة بنجاح إلى البريد الإلكتروني '" + email + "'. ");
				setTimeout(function(){ window.location.reload(); }, 1000);
            }
            else {
                $("#projectLoader").hide();
                showTopErrSucc('error', "حدث خطأ أثناء إرسال رابط الدعوة!");
            }
        }, 'json');
    } else {
        return false;
...
function setStatus(){
    $("td div[id^='allStatus']").each(function(res){
        ...
        var status = " العمل الجديد  <strong class='col-crt'>("+v_new+")</strong> <span>|</span> <strong class='col-wip'>("+v_replied+")</strong> العمل المتقدم <span>|</span>  <strong class='col-rslvd'>("+v_resolved+")</strong> العمل المنتهي <span>|</span><strong class='col-clsd'>("+v_closed+")</strong>  المغلق ";
        $("#"+id).html(status);
    });
}
...
function postSupport() {
    ...
    if (support_name == "") {
        ...
        if (!support_email.match(emailRegEx)) {
            $("#support_err").show();
            $("#support_err").html("يرجى إدخال البريد الإلكتروني الصحيح!");
            $("#support_email").focus();
            return false;
        } else if (support_msg == "") {
            $("#support_err").show();
            $("#support_err").html("لا يمكن ترك الرسالة فارغة!");
            $("#support_msg").focus();
            return false;
        } else {
            $("#spt_btn").hide();
            ...
            $.post(HTTP_ROOT + "users/post_support_inner", {
                ...
                if (data == "success") {
                    showTopErrSucc('success', 'شكرا لملاحظاتكم. وسوف نعود اليكم في أقرب وقت ممكن.');
                } else {
                    showTopErrSucc('error', "خطأ في مشاركة الدعم الفني!");
                }
                $("#spt_btn").show();
                $("#sprtloader").hide();
                $("#support_msg").val('');
...
function validateDailyMail() {
    ...
    if ($('.prj_users:checked').length == 0){
        errMsg = "الرجاء اختيار مستخدم واحد على الأقل.";
        done = 0;
    }else if($.trim($("#upd_hour").val()) == ''){
        errMsg = "الرجاء تحديد الساعة.";
        done = 0;
    }else if($.trim($("#upd_minute").val()) == ''){
        errMsg = "يرجى تحديد دقيقة.";
        done = 0;
    }
    if(done == 0) {
        showTopErrSucc('error',errMsg);
...
function cancel_daily_update() {
    if(confirm("هل تريد بالتأكيد إلغاء التنبيه اليومي لهذا المشروع؟")) {
        $('#subprof1').hide();
        $('#cancel_daily_update').hide();
        $('#subprof2').show();
...
function submitAddNewCase(postdata,CS_id,uniqid,cnt,dtls,status,prelegend,pid){
    ...
    if((res1 || res || res2 || res3 || res4 || res5 || res6 || res7) && !attachment){
        conf= confirm('هل تقصد إضافة ملف أو لقطة من الشاشة لهذه المهمة؟');
    }
    if(!conf){
    var CS_type_id = 2;
    var CS_priority = 1;
    ...
    if(CS_id) {
        ...
        if(!CS_id) {
            ...
            if(CS_title.trim() == "" || CS_title.trim() == "أضف المهمة هنا ثم إضغط على الرجوع") {
    ...
    if((done == 1 && emailUser.length != "0") || (done == 1 && confirm("هل تريد بالتأكيد المتابعة دون إبلاغ أي شخص؟"))) {
        if(!CS_id) {
        ...
        $.post(tskURL,{
            ...
            if(data) {
                if(!CS_id) {
                    ...
                    if(task_uid){
                        $("#t_"+task_uid).remove();
                        showTopErrSucc('success','تم تحديث مهمتك.');
                    }else{
                        showTopErrSucc('success','تم تأكيد نشر مهمتك.');
                    }
                    $('#drive_tr_0').remove();
                    $('#usedstorage').val('');
                    $('#up_files').empty();
                    ...
                    showTopErrSucc('success','تم نشر ردك.');
...
function blur_txt(){
    ...
    if($("#CS_title").val()!="أضف المهمة هنا ثم إضغط على الرجوع") {
        $("#CS_title").css({
            color:"#000000"
        });
    }
...
function checktitle_value(){
    ...
    if(tasktitle == "" || tasktitle == "أضف المهمة هنا ثم إضغط على الرجوع") {
    }else{
        $('#CS_title').css('border-color', '');
    }
}
...
function removefile(){
    ...
    if(val.length!='0'){
        if(confirm("هل أنت متأكد من أنك تريد إزالة؟")){
            document.getElementById('caseLoader').style.display="block";
            $.post(url,{
                "val":val
            }, function(data){
                if(data){
                    showTopErrSucc('success','تمت إزالة الملف.');
...
function getImNotifyMsg(projShName, caseNum, caseTtl, caseTyp){
    ...
    switch(caseTyp) {
        case 'NEW':
            action = "تم إنشاء مهمة جديدة";
            break;
        case 'UPD':
            action = "تم تحديث المهمة";
            break;
        case 'ARC':
            action = "تم أرشفة المهمة";
            break;
        case 'DEL':
...
function delMilestone(obj,name,uniqid) {
    ...
    if (confirm("هل تريد بالتأكيد حذف المعلم  '" + name + "' ?")){
...
function milestoneArchive(obj,uniqid,title) {
    ...
    if(confirm("هل تريد بالتأكيد إكمال هذا المعلم '" + title + "' ?")) {
...
function milestoneRestore(obj,uniqid,title) {
    ...
    if (confirm("هل تريد بالتأكيد استعادة هذا المعلم '" + title + "' ?")){
    ...
    if (done) {
        ...
        $.post(HTTP_ROOT+'milestones/assign_case', {
            ...
            if (data == "success") {
                ...
                $('#tskloader').hide();
                $("#confirmbtntsk").show();
                showTopErrSucc('success', 'تمت إضافة المهمة بنجاح.');
                if($('#caseMenuFilters').val()=='milestonelist'){
                    showMilestoneList();
                }else if($('#caseMenuFilters').val()=='milestone'){
                    ManageMilestoneList();
        ...
        showTopErrSucc('error', 'اختر مهمة لإضافتها في المعلم.');
    }
}
...
function iniChartTskProgress(id, res){
    ...
    if(!res.task_prog){
        if(id == 'task_type'){
            $("#"+id).html('<div class="mytask"></div><div class="mytask_txt">لا يوجد نوع المهمة</div>');
        } else {
            $("#"+id).html('<div class="mytask"></div><div class="mytask_txt">لا يوجد مهام</div>');
        }
        return;
    }
...
function deleteAssignedProject(id,userId,name,isInvite){
    if(id){
        var conf = confirm("هل تريد بالتأكيد حذف المشروع المعين '"+decodeURIComponent(name.replace(/\+/g,' '))+"' ?");
        if(conf == true){
...
function deleteUsersInProject(userId,projectId,name){
    if(userId){
        var conf = confirm("هل تريد بالتأكيد حذف المستخدم '"+decodeURIComponent(name.replace(/\+/g,' '))+"' من هذا المشروع؟");
        if(conf == true){
            var strURL = document.getElementById('pageurl').value;
            var dcrs_cnt = 1;
...
function removecaseFromMilestone(obj){
    if(confirm('هل تريد بالتأكيد إزالة هذه المهام المحددة من المعلم؟')){
        $("#mlstnpopup").hide();
        var caseid = Array();
        var done = 0;
        ...
        if (done) {
            ...
            $.post(HTTP_ROOT+'milestones/remove_case', {
                ...
                if (data == "success") {
                    ...
                    $('#tskloader').hide();
                    $("#confirmbtntsk").show();
                    showTopErrSucc('success', 'تمت إزالة المهمة بنجاح.');
                    if($('#caseMenuFilters').val()=='milestonelist'){
                        showMilestoneList();
                    }else if($('#caseMenuFilters').val()=='milestone'){
                        ManageMilestoneList();
            ...
            showTopErrSucc('error', 'اختر مهمة لإزالة من المعلم.');
        }
    }
}
...
function milstoneonTask(mlstname,mlstid){
    $.post(HTTP_ROOT+'milestones/milestone_list',{
        ...
        if(res){
            ...
            $('#more_opt8 ul').append('<li><a href="javascript:jsVoid()">&nbsp;&nbsp;لا معالم في المشروع</a></li>');
            $.each(res,function(key,value){
...
function validateTaskType() {
	...
	$("#task_type_nm").focus();
        ...
        if (!nm.match(/^[a-zA-Z0-9\u0600-\u06FF,-][\sa-zA-Z0-9\u0600-\u06FF,-]*$/g)) {
            msg = "'Name' يجب أن يبدأ مع بالأحرف الأبجدية أو الأرقام!";
	    $("#tterr_msg").show().html(msg);
	    $("#task_type_nm").focus();
            return false;
        }
    ...
    if (shnm === "") {
        msg = "'Short Name' لا يمكن ان يترك فارغا!";
	$("#tterr_msg").show().html(msg);
	...
	var x = shnm.substr(-1);
        if (!isNaN(x)) {
            msg = "'Short Name' لا يمكن أن ينتهي بالأرقام أو الفراغات!";
            $("#tterr_msg").show().html(msg);
	    $("#task_type_shnm").focus();
            return false;
        }
	 $.post(HTTP_ROOT+"projects/validateTaskType",{'id':id,'name':nm,'sort_name':shnm},function(data) {
		 ...
		 $("#ttloader").hide();
		 if(data.msg == 'name'){
		     $("#tterr_msg").show().html('الاسم موجود بالفعل !. الرجاء إدخال اسم آخر.');
		 }else if(data.msg == 'sort_name'){
		     $("#tterr_msg").show().html('الاسم المختصر موجود بالفعل !. الرجاء إدخال اسم مختصر آخر.');
		 }else{
...
function saveTaskType() {
	...
	showTopErrSucc('error','حدد على الأقل نوع مهمة واحد.');
	return false;
    }
}
...
function deleteTaskType(obj) {
   ...
   if (confirm("هل أنت متأكد أنك تريد حذف '"+nm+"' نوع المهمة؟")) {
	$("#del_tsk_"+id).hide();
	$("#lding_tsk_"+id).show();
	$.post(HTTP_ROOT+"projects/deleteTaskType",{"id": id},function(res) {
		...
		$("#dv_tsk_" + id).fadeOut(300, function(){
		    $(this).remove();
		    showTopErrSucc('success',"نوع المهمة '"+nm+"' تم حذفه بنجاح.");
		});
	    } else {
		 $("#lding_tsk_"+id).hide();
		 $("#del_tsk_"+id).show();
		 showTopErrSucc('error','حدث خطأ أثناء حذف نوع المهمة.');
	    }
	});
   }
}
