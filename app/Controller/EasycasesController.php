...
//        pr($resCaseProj);exit;
        ...
        $thursday = date('Y-m-d',strtotime($curCreated."next Thursday"));
        $sunday = date('Y-m-d',strtotime($curCreated."next Sunday"));
        ...
        $resCaseProj['mdyThursday'] = date('m/d/Y', strtotime($thursday));
        $resCaseProj['mdySunday'] = date('m/d/Y', strtotime($sunday));
...
//        pr($resCaseProj);exit;
        ...
        if($caseTypeRep == 10 || $caseLegendRep == 3 || $caseLegendRep == 5) {
            ...
            if($caseDuDate != "NULL" && $caseDuDate != "0000-00-00" && $caseDuDate != "" && $caseDuDate != "1970-01-01") {
                if($caseDuDate < $curdtT) {
                    $csDuDtFmtT = $dt->facebook_datestyle($caseDuDate);
                    $csDuDtFmt = '<div class="fl over-due"><span class="due-txt">متأخر</span><div class="cb" style="height:4px;"></div><div class="fl">'.$dt->dateFormatOutputdateTime_day($caseDuDate,$curDateTz,'week').'</div></div>';
                }else {
                    $csDuDtFmtT = $dt->facebook_datestyle($caseDuDate);
                    $csDuDtFmt = $dt->dateFormatOutputdateTime_day($caseDuDate,$curDateTz,'week');
	...
	$typeArr = $this->Type->query($sql);
        ...
        $thursday = date('Y-m-d',strtotime($curDateTz."next Thursday"));
        $sunday = date('Y-m-d',strtotime($curDateTz."next Sunday"));
        ...
        $caseDetail['mdyThursday'] = date('m/d/Y', strtotime($thursday));
        $caseDetail['mdySunday'] = date('m/d/Y', strtotime($sunday));
    ...
    function ajax_common_breadcrumb() {
        ...
        if(!empty($duedate)) {
            ...
            if(trim($duedate) == 'overdue') {
                $arr['duedate'] = "<div class='fl filter_opn' rel='tooltip' title='Due Date' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"duedate\");'>متأخر<a href='javascript:void(0);' onclick='common_reset_filter(\"duedate\",\"\",this);' class='fr'>X</a></div>";
            }else if(trim($duedate) == '24') {
                $arr['duedate'] = "<div class='fl filter_opn' rel='tooltip' title='Due Date' onclick='openfilter_popup(1,\"dropdown_menu_all_filters\");allfiltervalue(\"duedate\");'>اليوم<a href='javascript:void(0);' onclick='common_reset_filter(\"duedate\",\"\",this);' class='fr'>X</a></div>";
...
CompanyUser.company_id=".SES_COMP." $usercond) AS total_users $projectcond";
    ...
    function task_progress() {
        ...
        if($query_All) {
            ...
            if(!$newWipRate || $newWipRate==0.00) {
                $taskProg = array(
                        array( 'name'=>'منتهية','color'=>'#9FBD4B', 'y'=> $resRate),
                );
            } elseif(!$resRate || $resRate==0.00) {
                $taskProg = array(
                        array( 'name'=>'جديدة وفي تقدم','color'=>'#E1857A', 'y'=> $newWipRate),
                );
            } else {
                $taskProg = array(
                        array( 'name'=>'منتهية','color'=>'#9FBD4B', 'y'=> $resRate),
	...
	$this->layout = 'ajax';
        ...
        if($query_All) {
            ...
            if(!$newWipRate || $newWipRate==0.00) {
                $taskProg = array(
                        array( 'name'=>'مكتملة','color'=>'#9FBD4B', 'y'=> $resRate),
                );
            } elseif(!$resRate || $resRate==0.00) {
                $taskProg = array(
                        array( 'name'=>'جديدة وفي تقدم','color'=>'#E1857A', 'y'=> $newWipRate),
    ...
    function task_status() {
        ...
        $stsMsg = '';
        $stsMsgTtl = '';
        $stsArr = array(1=>'جديدة', 2=>'متقدمة', 3=>'مغلقة', 4=>'متقدمة', 5=>'منتهية');
        $stsColorArr = array('جديدة' => '#AE432E', 'متقدمة' => '#244F7A', 'مغلقة' => '#77AB13', 'منتهية' => '#EF6807');
