    ...
    function manage() {
		...
		if (isset($_GET['resend']) && trim(urldecode($_GET['resend'])) != "") {
			...
			if (!$getUser['User']['password']) {
				$subject =  " دعوة للمشاركة في ".$comp['Company']['name']." من إدارة التخطيط والمتابعة ";
				$ext_user = 1;
			} else {
				$subject = " دعوة للمشاركة في بنامج المهام من إدارة التخطيط والمتابعة ";
			}
		...
		if (isset($_GET['resetpassword']) && $_GET['resetpassword']) {
			...
			if (count($getData)) {
				...
				$message = "<table cellspacing='1' cellpadding='1'  width='100%' border='0'>
									<tr><td>&nbsp;</td></tr>
									<tr><td align='left' style='font:normal 14px verdana;'>،السلام عليكم " . $name . "</td></tr>
									<tr><td>&nbsp;</td></tr>
									<tr><td align='left' style='font:normal 14px verdana;'>تمت إعادة تعيين كلمة السر إلى <b>" . $newPasswrod . "</b></td></tr>
									<tr><td>&nbsp;</td></tr>
									<tr><td>&nbsp;</td></tr>
								</table>
								";
	...
	function forgotpassword() 
	{
		if(!empty($this->request->data) && empty($this->request->data['User']['repass']) && empty($this->request->data['User']['newpass'])) {
			...
			if($getUsrData && is_array($getUsrData) && count($getUsrData)) {
				...
				$this->Email->delivery = EMAIL_DELIVERY;
				$this->Email->to = $to;
				$this->Email->subject = "طلب إعادة تعيين كلمة السر";
				...
				if($this->Sendgrid->sendgridsmtp($this->Email))
				{
					...
					$this->Session->setFlash("يرجى التحقق من بريدك لإعادة تعيين كلمة السر", 'default', array('class'=>'success'));
					$this->redirect(HTTP_ROOT."users/forgotpassword/");
				}
			}
			else {
				//$this->Session->write("ERROR_RESET","<font style='color:red;'>إذا كان هناك حساب موجود بعنوان البريد الإلكتروني هذا، فقد أرسلنا تعليمات حول إعادة تعيين كلمة المرور. تفقد بريدك الالكتروني من فضلك</font>");
				$this->Session->setFlash("إذا كان هناك حساب موجود بعنوان البريد الإلكتروني هذا، فقد أرسلنا تعليمات حول إعادة تعيين كلمة السر. تفقد بريدك الالكتروني من فضلك", 'default', array('class'=>'success'));
				$this->redirect(HTTP_ROOT."users/forgotpassword/");
			}
		}
    ...
    public function login($demo = NULL,$email= NULL,$pass= NULL,$first_login=0) {
        ...
        if(!empty($this->request->data) || !empty($email)) {
			...
			else
			{
				...
				$this->Session->setFlash("البريد الإلكتروني أو كلمة السر غير صحيحين!", 'default', array('class'=>'error'));
...
//	}
	...
	function generateMsgAndSendUsMail($pjnames,$userid,$projUniqId,$comp)
	{
		...
		if(stristr($pjnames,",")) {
			$multiple = 1;
			$subject = "لقد تمت إضافتك إلى مشاريع متعددة في نظام إدارة المهام";
		}
		else {
			$subject = " في نظام إدارة المهام ".$pjnames." قام بإضافتك ل  ";
		}
