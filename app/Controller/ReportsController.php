...
/* BUG PIE CHART */
	...
	function bug_pichart(){
		...
		if(!empty($easycase)){
			...
			print "<div class='fl'><font color='red' size='2px'>لا توجد بيانات لهذا النطاق الزمني والمشروع</font></div>";exit;
		}
	}
...
/* BUG LINECHART */
	...
	function bug_linechart(){
		...
		if(!empty($case_arr)){
			...
			print "<font color='red' size='2px'>لا توجد بيانات لهذا النطاق الزمني والمشروع</font>";exit;
		}
 ...
 */
	...
	function tasks_trend(){
		...
		foreach($dt_arr as $key =>$date){
			...
			$typ = array(1=>'جديدة','2'=>'متقدمة',5=>'منتهية',3=>'مغلقة');
			...
			if(!$type_id){
				$carr =array(array('name'=>'جديدة','color'=>'#F90F0F', 'connectNulls'=> 'true','data'=>$bugs),array('name'=>'متقدمة','color'=>'#0066FF','connectNulls'=> 'true','data'=>$dev),array('name'=>'منتهية','color'=>'#DF6625','connectNulls'=> 'true','data'=>$resolved),array('name'=>'مغلقة','color'=>'#77AB13','connectNulls'=> 'true','data'=>$closed));
			}elseif($type_id==1){
				$carr =array(array('name'=>'جديدة','color'=>'#F90F0F', 'connectNulls'=> 'true','data'=>$bugs));
			}elseif($type_id==2){
				$carr =array(array('name'=>'متقدمة','d'=>'M 4 7 L 12 7 12 15 4 15 Z','color'=>'#0066FF','connectNulls'=> 'true','data'=>$dev));
			}elseif($type_id==5){
				$carr =array(array('name'=>'مغلقة','color'=>'#77AB13','connectNulls'=> 'true','data'=>$closed));
			}elseif($type_id==4){
				$carr =array(array('name'=>'منتهية','color'=>'#DF6625','connectNulls'=> 'true','data'=>$resolved));
			}
			for($i=5;$i<=100;$i++){
				$yarr[]=(int)$i;
			}
