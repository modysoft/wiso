    ...
    function formatActivities($activity, $total, $fmt, $dt, $tz, $csq){
		...
		//Assign value in variables.
			...
			if($caseDtMsg == ''){
				if($caseReplyType == 0){
					if($caseDtLegend == 1) {
						$msg = ' <span class="col-crt"><b>أنشأ المهمة </b></span> '.$eTitle;
					} elseif($caseDtLegend == 2 || $caseDtLegend == 4){
						$msg = ' <span class="col-wip"><b>وضع ملاحظة على المهمة </b> </span><span class="fnt_clr_gry"></span> '.$eTitle;
					} elseif($caseDtLegend == 3) {
						$msg = ' <span class="col-clsd"><b> المهمةأغلق</b></span> '.$eTitle;
					} elseif($caseDtLegend == 5){
						$msg = ' <span class="col-rslvd"><b>أنهى المهمة</b></span> '.$eTitle;
					} elseif($caseDtLegend == 6){
						$msg = ' <span class="col-rslvd"><b>عدل على المهمة</b></span> '.$eTitle;
					}			
					...
					$msg = ' <span class="col-wip"><b>حدث</b> </span><span class="fnt_clr_gry">نوع المهمة الى <b>'.$name."</b> on</span> ".$eTitle;
					...
					$msg = ' <span class="col-wip"><b>عين المهمة</b></span> '.$eTitle.' <span class="fnt_clr_gry">للتنفيذ من قبل <b>'.$by_name_assign.'</b>('.$short_name_assign.')</span>';
					...
					$msg = ' <span class="col-wip"><b>حدث</b> </span><span class="fnt_clr_gry">أهمية المهمة الى <b>'.$prio.'</b> للمهمة التالية</span> '.$eTitle;
					...
					if($caseDtDue != "NULL" && $caseDtDue != "0000-00-00" && $caseDtDue != "" && $caseDtDue != "1970-01-01") {
						...
						$msg ='<span class="fnt_clr_gry"> <b>'.$due_date.'</b></span>' .' <span class="col-wip"><b> حدث  </b> </span> <span class="fnt_clr_gry">تاريخ الإستحقاق للمهمة </span> '.$eTitle.' لتكون ' ;
					}
				}
			}else{			
				$msg = ' <span class="col-wip"><b> وضع ملاحظة جديدة </b> </span><span class="fnt_clr_gry">على المهمة </span> '.$eTitle;
			}
			} else {
				$msg = ' <span class="col-crt"><b>أنشأ المهمة </b></span> '.$eTitle;
			}
