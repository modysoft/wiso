	...
	foreach($allCases as $getdata) { 
		...
		<td style="font-weight:normal;<?php echo $class; ?>">
			...
			elseif($casePriority == 0){ 
				echo "<span class='tag red' style='width:5px;height:12px;' title='عالي' rel='tooltip'>&nbsp;</span>";
			}
			elseif($casePriority == 1){ 
				echo "<span class='tag orange' style='width:5px;height:12px;' title='متوسط' rel='tooltip'>&nbsp;</span>";
			}
			elseif($casePriority >= 2){ 
				echo "<span class='tag green' style='width:5px;height:12px;' title='منخفض' rel='tooltip'>&nbsp;</span>";
			}
			?>
		</td>
		<?php
...
<?php
	...
	?>
	<center style="font-weight:normal;color:#FF0000;padding:10px;">لا يوجد مهمات متاحة</center>
	<?php
}