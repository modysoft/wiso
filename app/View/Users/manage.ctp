...
<div class="tab tab_comon tab_task">
	    ...
	    <li id="task_li" <?php if($role == '' || $role == 'all'){?>class="active" <?php }?>>
               <a href="javascript:void(0);" onclick="filterUserRole('all','<?php echo $user_srch;?>');">
                ...
                <div class="fl"><span class="counter">(<?php echo $active_user_cnt;?>)</span> نشط</div>
                <div class="cbt"></div>
                </a>
            </li>
            <li id="file_li" <?php if($role == 'invited'){?>class="active" <?php }?>>
                ...
                <div class="fl"><span class="counter">(<?php echo $invited_user_cnt;?>)</span> مدعو للمشاركة</div>
                <div class="cbt"></div>
                </a>
            </li>
            <li id="task_li" <?php if($role == 'disable'){?>class="active" <?php }?>>
                ...
                <div class="fl"><span class="counter">(<?php echo $disabled_user_cnt;?>)</span>غير مفعل </div>
                <div class="cbt"></div>
                </a>
            </li>
            <div class="cbt"></div>
...
<div class="col-lg-12 user_div m-left-20">
    ...
    <div class="col-lg-4 proj_mng_div">
		...
		<?php if ($user['CompanyUser']['user_type'] == 1) { ?>
		    <div class="fl">
			<a class="icon-assign-usr" href="javascript:void(0);" data-usr-id="<?php echo $user['User']['id'];?>" data-usr-name="<?php echo $user['User']['name'];?>">تعيين مشروع</a>
		    </div>
		    <div class="fr">
				<input id="rmv_allprj_<?php echo $user['User']['id'];?>" type="hidden" value="<?php echo $user['User']['all_projects'];?>"/>
				<a id="rmv_prj_<?php echo $user['User']['id'];?>" class="icon-remprj-usr" href="javascript:void(0);" data-usr-id="<?php echo $user['User']['id'];?>" data-usr-name="<?php echo $user['User']['name'];?>" data-total-project="<?php echo $user['User']['total_project'];?>" <?php if($user['User']['all_project'] == ''){ ?> style="display:none;" <?php } ?>>حذف المشروع </a>
			</div>
		<?php } else {
		    if($role == 'invited') { ?>
			...
			<div class="fl">
			    <a class="icon-assign-usr" href="javascript:void(0);" data-usr-id="<?php echo $user['User']['id'];?>" data-usr-name="<?php echo $user['User']['email'];?>">تعيين مشروع</a>
			    <input id="rmv_allprj_<?php echo $user['User']['id'];?>" type="hidden" value="<?php echo $user['User']['all_projects'];?>"/>
			    <span id="rmv_prj_<?php echo $user['User']['id'];?>" <?php if($user['User']['all_project'] == ''){ ?> style="display:none;"<?php } ?>>
				    <br />
				<a class="icon-remprj-usr" href="javascript:void(0);" data-usr-id="<?php echo $user['User']['id'];?>" data-usr-name="<?php echo $user['User']['email'];?>" data-total-project="<?php echo $user['User']['total_project'];?>">حذف المشروع</a>
			    </span>	
			    
			</div>
			<div class="fr">
			    <a class="icon-delete-usr fl" href="<?php echo HTTP_ROOT; ?>users/manage/?del=<?php echo urlencode($user['User']['uniq_id']); ?>&role=<?php echo $_GET['role']; ?>" Onclick="return confirm(' \'<?php echo $user['User']['email']; ?>\' هل أنت متأكد أنك تريد حذف ؟')">حذف</a> <br/>
			    <a class="icon-resend-usr fl" href="javascript:void(0);" onclick="return resend_invitation('<?php echo $user['User']['qstr']; ?>','<?php echo $user['User']['email']; ?>');">إعادة إرسال</a>
			</div>
		   <?php } elseif($role == 'disable'){ ?>
			<div class="fl">
			    <a class="icon-enable-usr fl" href="<?php echo HTTP_ROOT; ?>users/manage/?act=<?php echo urlencode($user['User']['uniq_id']); ?>&role=<?php echo $_GET['role']; ?>" Onclick="return confirm(' \'<?php echo $user['User']['name']; ?>\' هل تريد بالتأكيد تفعيل ؟')">تفعيل</a>
			</div>
			<div class="fr">
				<input id="rmv_allprj_<?php echo $user['User']['id'];?>" type="hidden" value="<?php echo $user['User']['all_projects'];?>"/>
				<a id="rmv_prj_<?php echo $user['User']['id'];?>" class="icon-remprj-usr" href="javascript:void(0);" data-usr-id="<?php echo $user['User']['id'];?>" data-usr-name="<?php echo $user['User']['name'];?>" data-total-project="<?php echo $user['User']['total_project'];?>" <?php if($user['User']['all_project'] == ''){ ?> style="display:none;" <?php } ?>>حذف المشروع </a>
		  ...
		  <?php } else { ?>
			<div class="fl">
			    <a class="icon-assign-usr" href="javascript:void(0);" data-usr-id="<?php echo $user['User']['id'];?>" data-usr-name="<?php echo $user['User']['name'];?>">تعيين المشروع</a>
			    <input id="rmv_allprj_<?php echo $user['User']['id'];?>" type="hidden" value="<?php echo $user['User']['all_projects'];?>"/>
				<span id="rmv_prj_<?php echo $user['User']['id'];?>" <?php if($user['User']['all_project'] == ''){ ?> style="display:none;"<?php } ?>>
					<br />
				    <a class="icon-remprj-usr" href="javascript:void(0);" data-usr-id="<?php echo $user['User']['id'];?>" data-usr-name="<?php echo $user['User']['name'];?>" data-total-project="<?php echo $user['User']['total_project'];?>">حذف المشروع</a>
				</span>
			</div>
			<div class="fr">
			    <a class="icon-disable-usr" href="<?php echo HTTP_ROOT; ?>users/manage/?deact=<?php echo urlencode($user['User']['uniq_id']); ?>" Onclick="return confirm('هل تريد بالتأكيد تعطيل ؟ \'<?php echo $user['User']['name']; ?>\'')">تعطيل</a><br />
			    <?php  if($istype == 1) {
				if ($user['CompanyUser']['user_type'] == 2) {?>
			    <a class="icon-revadmin-usr" href="<?php echo HTTP_ROOT; ?>users/manage/?revoke_admin=<?php echo urlencode($user['User']['uniq_id']); ?>" Onclick="return confirm('هل تريد بالتأكيد إلغاء امتياز المشرف من ؟ \'<?php echo $user['User']['name']; ?>\'')">إبطال المشرف</a>
			    <?php } else {?>
			    <a class="icon-admin-usr" href="<?php echo HTTP_ROOT; ?>users/manage/?grant_admin=<?php echo urlencode($user['User']['uniq_id']); ?>" Onclick="return confirm('هل تريد بالتأكيد منح امتياز المشرف ل ؟ \'<?php echo $user['User']['name']; ?>\'')">تعيين كمشرف</a>
			    <?php } }?>
			</div>
		    <?php }
		    } ?>
...
<div id="projectLoader">
    <div class="loadingdata">جار إرسال الدعوة مرة أخرى ...</div>
</div>