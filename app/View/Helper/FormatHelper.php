...
class FormatHelper extends AppHelper {
	...
	function getStatus($type,$legend){
		if($type == 10) {
			return '<div class="label update">محدثة</div>';
		} else if($legend == 1) {
			return '<div class="label new">جديدة</div>';
		} else if($legend == 2 || $legend == 4) {
			return '<div class="label wip">في تقدم</div>';
		}
		if($legend == 3) {
			return '<div class="label closed">مغلقة</div>';
		} else if($legend == 4) {
			return '<div class="label wip">متقدمة</div>';
		} else if($legend == 5) {
			return '<div class="label resolved">منتهية</div>';
		}
	}
