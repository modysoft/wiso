...
<table class="expt-tbl">
	...
	<td align="left" valign="top" style="padding-top:10px;">
		...
		<option value="all" <?php if(!$priority || trim($priority) == 'all'){?>selected="selected"<?php } ?>>الكل</option>
		<option value="2" <?php if(trim($priority) == 'Low'){?>selected="selected"<?php } ?>>منخفض</option>
		<option value="1" <?php if(trim($priority) == 'Medium'){?>selected="selected"<?php } ?>>متوسط</option>
		<option value="0" <?php if(trim($priority) == 'High'){?>selected="selected"<?php } ?>>عالي</option>
	    </select>
	</td>
    </tr>
    <tr id="tr_members">
