...
<?php if(CONTROLLER == "easycases" && (PAGE_NAME == "dashboard")) {?>
	<li><span id="brdcrmb-cse-hdr">المهام</span></li>
<?php } ?>
...
<?php if((CONTROLLER == "easycases" && (PAGE_NAME == "dashboard" || PAGE_NAME == "mydashboard")) || (CONTROLLER == "milestones" && (PAGE_NAME == "milestone" || PAGE_NAME=='milestonelist')) || (CONTROLLER == "users" && (PAGE_NAME == "activity"))) {?>
	...
	<?php if((count($getallproj) == 0) && (SES_TYPE == 1 || SES_TYPE == 2) ) { ?>
		<a onclick="newProject()" href="javascript:void(0);"> <i style="color: 2D678D; font-weight: bold;"> إنشاء مشروع</i></a>
	<?php }else{
		 if(count($getallproj)=='0'){ ?>
		    <i style="color:#FF0000">لا يوجد</i>
	<?php } else {
...
<?php if(CONTROLLER == "reports" && (PAGE_NAME == "glide_chart" || PAGE_NAME == "chart" || PAGE_NAME == "hours_report")) { ?>
	...
	<?php if((count($getallproj) == 0) && (SES_TYPE == 1 || SES_TYPE == 2) ) { ?>
		<a onclick="newProject()" href="javascript:void(0);"><i style="color: 2D678D; font-weight: bold;"> إنشاء مشروع</i></a>
		<!--<button onclick="newProject('menupj','loaderpj');">إنشاء مشروع</button>-->
	<?php }else{
		 if(count($getallproj)=='0'){ ?>
		    --لا يوجد--
	<?php } else {
	...
	<a href="<?php echo HTTP_ROOT.'dashboard#tasks';?>" onclick="checkHashLoad('tasks');"><div id="lview_btn" class="btn gry_btn kan30" title="رؤية كجدول"><i class="icon-list-view"></i></div></a>
	<a href="<?php echo HTTP_ROOT.'dashboard#tasks';?>" onclick="checkHashLoad('compactTask');"><div id="cview_btn" class="btn gry_btn kan30" title="رؤية موجزة"><i class="icon-compact-view"></i></div></a>
	<a href="<?php echo HTTP_ROOT.'dashboard#kanban';?>" onclick="checkHashLoad('kanban');"><div id="kbview_btn" class="btn gry_btn kan30" style="border-radius:0 3px 3px 0"  title=" رؤية كانبان"><i class="icon-kanv-view"></i></div></a>
	<a href="<?php echo HTTP_ROOT.'dashboard#activities';?>" onclick="checkHashLoad('activities');"><div id="actvt_btn" class="btn gry_btn kan30" style="border-radius:0 3px 3px 0"  title="الأنشطة"><i class="icon-actvt-view"></i></div></a>
	<a href="<?php echo HTTP_ROOT.'dashboard#calendar';?>" onclick="calendarView('calendar');"><div id="calendar_btn" class="btn gry_btn kan30" style="border-radius:0 3px 3px 0"  title="التقويم"><img src="<?php echo HTTP_ROOT; ?>img/calendar.png" style="margin-top:-8px;margin-left:-2px"></img></div></a>
	</li>
	<?php } ?>
	<li  class="kanbn dashborad-view-type" id="select_view_mlst" style="display: none;">
		<a href="<?php echo HTTP_ROOT.'dashboard#milestone';?>" onclick="checkHashLoad('milestone');" ><div id="mlview_btn" class="btn gry_btn kan30" title="إدارة المعلم"><i class="icon-list-view"></i></div></a>
		...
		<!--<a href="javascript:void(0);" onclick="addEditMilestone(this);" id="mlist_crt_mlstbtn" class="mlstlink_new" data-name="" data-uid="" data-id="">إنشاء معلم في المشروع</a>-->
		<button style="margin-left:25px;" onclick="addEditMilestone(this);" id="mlist_crt_mlstbtn" type="button" value="Create Milestone" class="btn btn_blue">   إنشاء معلم في المشروع   </button>
	</li>
