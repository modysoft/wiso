						...
						<tr>
							...
							<td>
								<div id="pridiv<%= csAtId %>" data-priority ="<%= csPriRep %>" class="pri_actions <%= protyCls %><% if(showQuickAct==1){ %> dropdown<% } %>">
									...
									<ul class="dropdown-menu quick_menu">
										<li class="pop_arrow_new"></li>
										<li><a href="javascript:void(0);" class="low_priority" onclick="detChangepriority(<%= '\'' + csAtId + '\', \'2\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">منخفض</a></li>
										<li><a href="javascript:void(0);" class="medium_priority" onclick="detChangepriority(<%= '\'' + csAtId + '\', \'1\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">متوسط</a></li>
										<li><a href="javascript:void(0);" class="high_priority" onclick="detChangepriority(<%= '\'' + csAtId + '\', \'0\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">مرتفع</a></li>
									</ul>
									<% } %>
								</div>
				 ...
				 <div class="col-lg-4 task_elements">
					<table cellpadding="0" cellspacing="0">
						<?php /*?><tr>
							<td>Milestone:</td>
							<td><b>
							<%= mistn?shortLength(mistn,20):'<i class="no_due_dt">لا يوجد</i>' %>
							</b></td>
						</tr><?php */?>
						<tr>
							<td>Project:</td>
		...
		<div class="reply_task_block" id="reply_box<%= csAtId %>">
			...
			<div class="fl col-lg-10">
				...
				<div class="col-lg-12 m-top-20">
					...
					<div class="fr mor_toggle tasktoogle" style="float:left" id="mor_toggle<%= csAtId %>" data-csatid="<%= csAtId %>"><a href="javascript:jsVoid();" style="text-decoration:none"><img src="<?php echo HTTP_IMAGES;?>priority.png" title="Priority" rel="tooltip"/>&nbsp;&nbsp;<img src="<?php echo HTTP_IMAGES;?>hours.png" title="عدد الساعات % منجز" rel="tooltip"/>&nbsp;&nbsp;<img src="<?php echo HTTP_IMAGES;?>attachment.png" title="Attachments, Google Drive, Dropbox" rel="tooltip"/>&nbsp;&nbsp;إختيرات أكثر<b class="caret"></b></a></div>
					<div class="fr less_toggle tasktoogle" id="less_toggle<%= csAtId %>" data-csatid="<%= csAtId %>" style="display:none;float:left"><a href="javascript:jsVoid();" style="text-decoration:none">أقل<b class="caret"></b></a></div>
				</div>
				...
				<div class="col-lg-12 m-top-20">
					...
					<span id="postcomments<%= csAtId %>">
						<button class="btn btn_blue" type="button" name="data[Easycase][postdata]" onclick="return validateComments(<%= '\''+csAtId+'\',\''+csUniqId+'\',\''+csLgndRep+'\',\''+SES_TYPE+'\',\''+csProjIdRep+'\'' %>);"><i class="icon-big-tick"></i>أرسل</button>
                        <span class="or_cancel"></span>
						<button class="task_detail_back or_cancel" type="reset" id="rset"><i class="icon-big-cross"></i>إلغاء</button>
					</span>
		...
		<?php /*?><div class="reply_task_block">
			...
			<div class="fl col-lg-10">
				...
				<div class="col-lg-12 m-top-20">				
					...
					<button class="btn btn_grey" type="reset" id="rset" onclick="closePopup();"><i class="icon-big-cross"></i>إلغاء</button>
					<a href="javascript:jsVoid();" onclick="slide_form_more();"><div class="fr mor_toggle">خيرات أكثر<b class="caret"></b></div></a>
					<a href="javascript:jsVoid();" onclick="slide_form_less();"><div class="fr less_toggle" style="display:none">خيرات أقل<b class="caret"></b></div></a>
				</div>				
			</div>
			<div class="cb"></div>
		</div><?php */?>
	...
	<div class="col-lg-3 fl col_task case_det_rt">
		...
		<div>
			<div class="asign_block">
				...
				<div id="case_dtls_asgn<%= csAtId %>" class="fl asgn_actions <% if(showQuickAct==1){ %> dropdown<% } %>">
					<span <% if(showQuickAct==1){ %> class="quick_action" data-toggle="dropdown"<% } %> onclick="displayAssignToMem(<%= '\'' + csAtId + '\'' %>, <%= '\'' + projUniqId + '\'' %>,<%= '\'' + asgnUid + '\'' %>,<%= '\'' + csUniqId + '\'' %>,<%= '\'details\'' %>,<%= '\'' + csNoRep + '\'' %>)">تم التعيين ل</span>
					<% if(showQuickAct==1){ %>
		...
		<div class="task_due_dt">
			...
			<div id="case_dtls_due<%= csAtId %>" class="fl due_actions <% if(showQuickActDD==1){ %> dropdown<% } %>">
				...
				<% } else { %>
				<div <% if(showQuickActDD==1){ %> class="no_due_dt duequick_action" data-toggle="dropdown"<% } else { %> class="fl no_due_dt" <% } %>><span class="due-txt">لا تاريخ استحقاق</span></div>
				<% } %>
				<% if(showQuickActDD==1){ %>
				<ul class="dropdown-menu quick_menu">
					<li class="pop_arrow_new"></li>
					<li><a href="javascript:void(0);" onclick="detChangeDueDate(<%= '\'' + csAtId + '\', \'00/00/0000\', \'لا تاريخ استحقاق\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">لا تاريخ استحقاق</a></li>
					<li><a href="javascript:void(0);" onclick="detChangeDueDate(<%= '\'' + csAtId + '\', \'' + mdyCurCrtd + '\', \'اليوم\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">اليوم</a></li>
					<li><a href="javascript:void(0);" onclick="detChangeDueDate(<%= '\'' + csAtId + '\', \'' + mdyTomorrow + '\', \'غداً\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">غداً</a></li>
					<li><a href="javascript:void(0);" onclick="detChangeDueDate(<%= '\'' + csAtId + '\', \'' + mdySunday + '\', \'الأحد المقبل\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">الأحد المقبل</a></li>
					<li><a href="javascript:void(0);" onclick="detChangeDueDate(<%= '\'' + csAtId + '\', \'' + mdyThursday + '\', \'هذا الخميس\', \'' + csUniqId + '\', \'' + csNoRep + '\'' %>)">هذا الخميس</a></li>
					<li>
						<a href="javascript:void(0);" class="cstm-dt-option" data-csatid="<%= csAtId %>">
							<input value="" type="hidden" id="det_set_due_date_<%= csAtId %>" class="set_due_date" title="حدد التاريخ" style=""/>
		...
		for(var fkey in all_files){
			...
			<div class="fr ftsk_more">
				<a class="more_in_menu" href="javascript:;">أكثر</a><b class="menu_more_arr file_more"></b>
			</div>
		<% }
		} if(fc==0) { %>
		<div class="fl">
