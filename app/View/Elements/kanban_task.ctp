	...
	<div class="dropdown fl">
			...
			<ul class="dropdown-menu sett_dropdown-caret" style="margin-top:5px;padding:5px 0;">
				...
				<li  onClick="addTaskToMilestone(<%= '\'\',\''+ mlstId + '\'' %>,<%= '\'' + mlstProjId+ '\'' %>,<%= '\'' + count + '\'' %>)" >
					<a href="javascript:void(0);"><div class="ct_icon icon-add-task-milston fl"></div><div class="fl mntxt">أضافة مهمة</div></a>
				</li>
				<li onclick="addEditMilestone(<%= '\'\',\'' + mlstUid + '\'' %>,<%= '\'' + mlstId + '\'' %>,<%= '\'' + mlstTitle + '\',1' %>)" class="makeHover">
					<a href="javascript:void(0)"><div class="ct_icon act_edit_task fl"></div>تعديل </a>
				</li>
				...
				<% if(mlsttype=='0'){ %>
					<li onclick="milestoneRestore(<%= '\'\',\'' + mlstUid + '\'' %>, <%= '\'' + mlstTitle + '\'' %>);"  >
						<a href="javascript:jsVoid();"><div class="ct_icon mt_completed fl"></div> <div class="fl mntxt" style="margin-left:-6px">إسترداد</div></a>
					</li>
				<% }else{ %>
					<li onclick="milestoneArchive(<%= '\'\',\'' + mlstUid + '\'' %>, <%= '\'' + mlstTitle + '\'' %>);"  >
						<a href="javascript:jsVoid();"><div class="ct_icon mt_completed fl"></div> <div class="fl mntxt" style="margin-left:-6px">أنهاء</div></a>
					</li>
				<% } %>
...
<div class="kanban-main">
	...
	for(var taskallkey in caseAll){
		...
		var tasktype='';
		if(taskallkey=='newTask'){tasktype='مهام جديـدة';}else if(taskallkey=='inprogressTask'){tasktype='مهام متقدمـــة';}else if(taskallkey=='resolvedTask'){tasktype='مهام منتهـية'}else if(taskallkey=='closedTask'){tasktype='مهام مغلقـة'};
	 if(!morecontent){
	%>
