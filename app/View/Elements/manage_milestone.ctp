		...
		<div class="col-lg-4 proj_mng_div">
			<div class="col-lg-12 contain usr_mng_div milestone_div">
                ...
                <div class="user-details prj_details">
                    ...
                    <div class="fl usr_rt">
                    <div class="border_usr"><b><span id="tot_tasks<%= milestoneAutoId %>"><%= total_cases %></span></b> المهام</div>
...
<!--				<div class="tsk_updts">
				    <span id="tot_tasks<%= milestoneAutoId %>"><%= total_cases %></span> المهام&nbsp; . &nbsp;<%= closed_cases %> مغلق
				</div>
...
-->			
                ...
                <div class="proj_mng">
				    <div class="fl">
                                        <% if(mlsttype!='0'){ %>
					<a href="javascript:void(0);" class="icon-add-task fl" onClick="addTaskToMilestone(this);" data-prj-id="<%= projId %>" data-id="<%= milestoneAutoId %>">إضافة مهمة</a>
					<br /> <% } %>
					<% if(mlsttype=='0'){ %>
						<a href="javascript:void(0);" class="icon-restore-mlstn fl" onClick="milestoneRestore(this);" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>">إستعادة</a>
					<% }else{ %>
						<a href="javascript:void(0);" class="icon-complete-mlstn fl" onClick="milestoneArchive(this);" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>">إكمال</a>
					<% } %>
					<% if(total_cases>0 && mlsttype!='0'){ %><br/>
						<a href="javascript:void(0);" class="icon-remov-task fl" onClick="removeTaskFromMilestone(this);" data-prj-id="<%= projId %>" data-id="<%= milestoneAutoId %>">إزالة المهمة</a>
					<% } %>
				    </div>
				    <div class="fr">
                                        <% if(mlsttype!='0'){ %>
					<a href="javascript:void(0);" class="icon-edit-mlstn fl" onClick="addEditMilestone(this);" data-id="<%= milestoneAutoId %>" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>">تعديل</a>
					<br /> <% } %>
					<a href="javascript:void(0);" class="icon-delete-mlstn fl" onClick="delMilestone(this);" data-uid="<%= milestoneUniqId %>" data-name="<%= milestoneTitle %>">حذف</a>
				    </div>
				</div>
			</div>
		</div>
