		...
		<td class="fnt12" valign="top">
			<div class="dropdown fl">
				...
				<ul class="dropdown-menu dudt_dropdown-caret">
					...
					<li><a href="javascript:void(0);" onclick="changeCaseDuedate(<%= '\'' + caseAutoId + '\', \'' + caseNo + '\'' %>); changeDueDate(<%= '\'' + caseAutoId + '\', \'' + mdySunday + '\', \'الأحد القادم\', \'' + caseUniqId + '\'' %>)">الأحد القادم</a></li>
	...
	<tr>
		<td colspan="5" align="center" style="padding:10px 0;color:#FF0000">
			...
			<% }else if(case_type == 'overdue'){ %>
				لا يوجد مهمات متأخرة
			<% }else if(case_type == 'delegateto'){ %>
				لا يوجد مهمات تحت التفويض
			<% }else if(case_type == 'highpriority'){ %>
				لا بوجد مهمات عالية الأهمية
			<% } %>
		</td>
	</tr>
<% } %>
