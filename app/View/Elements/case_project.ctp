...
<table width="100%" class="tsk_tbl compactview_tbl">
        ...
        <td width="18%" class="all_td">
            <div class="dropdown fl">
                ...
                <ul class="dropdown-menu" id="dropdown_menu_chk">
                    <li><a href="javascript:void(0);" onclick="multipleCaseAction(<%= '\'caseStart\'' %>)"><div class="act_icon act_start_task fl" title="Start"></div>البدء</a></li>
                    <li><a href="javascript:void(0);" onclick="multipleCaseAction(<%= '\'caseResolve\'' %>)"><div class="act_icon act_resolve_task fl" title="Resolve"></div>إنهاء</a></li>
                    <li><a href="javascript:void(0);" onclick="multipleCaseAction(<%= '\'caseId\'' %>)"><div class="act_icon act_close_task fl" title="Close"></div>إغلاق</a></li>
                    <?php if(SES_TYPE == 1 || SES_TYPE == 2) {?>
                    <li id="mvTaskToProj"><a href="javascript:void(0);" onclick="mvtoProject(<%= '\' \'' %>,<%= '\' \'' %>,<%= '\'movetop\'' %>)"><div class="act_icon pro_mov fl" title="Move to roject"></div>إنقل الى المشروع</a></li>
                        <?php } ?>
                </ul>
            </div>
        </td>
    ...
    <tr class="tr_all" id="curRow<%= caseAutoId %>">
        ...
        <td <% if(groupby =='' || groupby !='priority'){%>class="pr_<%= easycase.getPriority(casePriority) %>"<% } %> valign="top">
            ...
            <div class="dropdown fl">
                ...
                <ul class="dropdown-menu sett_dropdown-caret">
                    ...
                    <li onclick="startCase(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + caseUniqId + '\'' %>);" id="start<%= caseAutoId %>" style=" <% if(caseFlag == "1"){ %>display:block<% } else { %>display:none<% } %>">
                        <a href="javascript:void(0);"><div class="act_icon act_start_task fl" title="Start"></div>إبدأ</a>
                    </li>
                    <% }
                    ...
                    <li onclick="caseResolve(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + caseUniqId + '\'' %>);" id="resolve<%= caseAutoId %>" style=" <% if(caseFlag == 2){ %> display:block <% } else { %> display:none <% } %>">
                        <a href="javascript:void(0);"><div class="act_icon act_resolve_task fl" title="Resolve"></div>إنهاء</a>
                    </li>
                    <% }
                    ...
                    <li onclick="setCloseCase(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + caseUniqId + '\'' %>);" id="close<%= caseAutoId %>" style=" <% if(caseFlag == 5) { %>display:block <% } else { %>display:none<% } %>">
                        <a href="javascript:void(0);"><div class="act_icon act_close_task fl" title="Close"></div>إغلاق</a>
                    </li>
                    ...
                    <li id="act_reply<%= count %>" data-task="<%= caseUniqId %>">
                        <a href="javascript:void(0);" id="reopen<%= caseAutoId %>" style="<% if(caseFlag == 7){ %>display:block <% } else { %>display:none<% } %>"><div class="act_icon act_reply_task fl" title="Re-open"></div>إعادة فتح</a>
                        <a href="javascript:void(0);" id="reply<%= caseAutoId %>" style="<% if(caseFlag == 8){ %>display:block <% } else { %>display:none<% } %>"><div class="act_icon act_reply_task fl" title="Reply"></div>أرسل</a>
                    </li>
                    <% }
                    ...
                    <li onclick="editask(<%= '\''+ caseUniqId+'\',\''+projectUniqid+'\',\''+projectName+'\'' %>);" id="edit<%= caseAutoId %>" style=" <% if(caseFlag == 3 || SES_TYPE == 1 || SES_TYPE == 2){ %>display:block <% } else { %>display:none<% } %>">
                        <a href="javascript:void(0);"><div class="act_icon act_edit_task fl" title="Edit"></div>تعديل</a>
                    </li>
                    <% }
					if((caseLegend == 1 || caseLegend == 2 || caseLegend == 4) && caseTypeId!= 10) { caseFlag=2; }
		    ...
		    <a href="javascript:void(0);"><div class="act_icon pro_mov fl" title="Move to Project"></div>إنقل الى المشروع</a>
		    </li>
		    <% } %>
		    <% if(getdata.Easycase.isactive == 0){ %>
			<li data-prjid="<%= projId %>" data-caseid="<%= caseAutoId %>" data-caseno="<%= caseNo %>"  id="mv_prj<%= caseAutoId %>" style=" ">
				<a onclick="restoreFromTask(<%= caseAutoId %>,<%= projId %>,<%= caseNo %>)" href="javascript:void(0);"><div class="act_icon act_restore_task fl" title="Restore"></div>إسترداد</a>
			</li>
			<li data-prjid="<%= projId %>" data-caseid="<%= caseAutoId %>" data-caseno="<%= caseNo %>"  id="mv_prj<%= caseAutoId %>" style=" ">
			    <a onclick="removeFromTask(<%= caseAutoId %>,<%= projId %>,<%= caseNo %>)" href="javascript:void(0);"><div class="act_icon act_del_task fl" title="Remove"></div>إزالة</a>
			</li>
		    <% } %>
                    <% }
					if(getdata.Easycase.isactive == 1){ %>
                    <li onclick="moveTask(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'\'' %>,<%= '\'' + projId + '\'' %>);" id="moveTask<%= caseAutoId %>" style=" <% if(caseFlag == 2){ %> display:block <% } else { %> display:none <% } %>">
                        <a href="javascript:void(0);"><div class="act_icon task_move_mlst fl" title="Move Task To Milestone"></div>إنقل الى معلم</a>
                    </li>
                    <% } %>
                    ...
                    <li onclick="removeThisCase(<%= '\'' + count + '\'' %>,<%= '\'' + getdata.Easycase.Emid + '\'' %>, <%= '\'' + caseAutoId + '\'' %>, <%= '\'' + getdata.Easycase.Em_milestone_id + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + caseUserId + '\'' %>);" id="rmv<%= caseAutoId %>" style="<% if(caseFlag == "remove"){ %>display:block<% } else { %>display:none<% } %>">
                        <a href="javascript:void(0);"><div class="act_icon act_rmv fl" title="Remove Task"></div>إزالة المهمة</a>
                    </li>
                    <%
					}
					}
                    ...
                    <li onclick="archiveCase(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + projId + '\'' %>, <%= '\'t_' + caseUniqId + '\'' %>);" id="arch<%= caseAutoId %>" style="<% if(caseFlag == "archive"){ %>display:block<% } else { %>display:none<% } %>">
                        <a href="javascript:void(0);"><div class="act_icon act_arcv_task fl" title="Archive"></div>أرشفة</a>
                    </li>
                    <% }
                    ...
                    <li onclick="deleteCase(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + projId + '\'' %>, <%= '\'t_' + caseUniqId + '\'' %>);" id="arch<%= caseAutoId %>" style="<% if(caseFlag == "delete"){ %>display:block<% } else { %>display:none<% } %>">
                        <a href="javascript:void(0);"><div class="act_icon act_del_task fl" title="Delete"></div>حذف</a>
                    </li>
                    <% } %>
                </ul>
            </div>
        ...
        <td class="fnt12" valign="top">
            <div class="dropdown fl">
                ...
                <ul class="dropdown-menu dudt_dropdown-caret">
                    <li class="pop_arrow_new"></li>
                    <li><a href="javascript:void(0);" onclick="changeCaseDuedate(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>);changeDueDate(<%= '\'' + caseAutoId + '\', \'00/00/0000\', \'لا تاريخ استحقاق\', \'' + caseUniqId + '\'' %>)">لا تاريخ استحقاق</a></li>
                    <li><a href="javascript:void(0);" onclick="changeCaseDuedate(<%= '\'' + caseAutoId + '\', \'' + caseNo + '\'' %>); changeDueDate(<%= '\'' + caseAutoId + '\', \'' + mdyCurCrtd + '\', \'اليوم\', \'' + caseUniqId + '\'' %>)">اليوم</a></li>
                    <li><a href="javascript:void(0);" onclick="changeCaseDuedate(<%= '\'' + caseAutoId + '\', \'' + caseNo + '\'' %>); changeDueDate(<%= '\'' + caseAutoId + '\', \'' + mdyTomorrow + '\', \'غداً\', \'' + caseUniqId + '\'' %>)">غداً</a></li>
                    <li><a href="javascript:void(0);" onclick="changeCaseDuedate(<%= '\'' + caseAutoId + '\', \'' + caseNo + '\'' %>); changeDueDate(<%= '\'' + caseAutoId + '\', \'' + mdySunday + '\', \'الأحد المقبل\', \'' + caseUniqId + '\'' %>)">الأحد المقبل</a></li>
                    <li><a href="javascript:void(0);" onclick="changeCaseDuedate(<%= '\'' + caseAutoId + '\', \'' + caseNo + '\'' %>); changeDueDate(<%= '\'' + caseAutoId + '\', \'' + mdyThursday + '\', \'هذا الخميس\', \'' + caseUniqId + '\'' %>)">هذا الخميس</a></li>
                    <li>
                        <a href="javascript:void(0);" class="cstm-dt-option" data-csatid="<%= caseAutoId %>">
                            <input value="" type="hidden" id="set_due_date_<%= caseAutoId %>" class="set_due_date" title="تحديد التاريخ" style=""/>
